...
import time
import pandas as pd
from utils.load import load_ACASXU
...
def run_acas_experiment(
    x: int,
    y: int,
    spec_id: int,
    coef:float=0.3,
    uniform_ratio: float = 0.25,
    result_dir: str = "./results/acas_experiments",
    result_prefix: str = "acas_parallel_results",
    parallel: bool = True
) -> pd.DataFrame:
    """
...
def run_acas_experiment_parallel(x, y, spec_id, coef=0.3, uniform_ratio=0.25, result_dir="./results/acas_experiments", result_prefix="acas_parallel_results"):
    return run_acas_experiment(x, y, spec_id, coef, uniform_ratio, result_dir, result_prefix, parallel=True)

def run_acas_experiment_serial(x, y, spec_id, coef=0.3, uniform_ratio=0.25, result_dir="./results/acas_experiments", result_prefix="acas_serial_results"):
    return run_acas_experiment(x, y, spec_id, coef, uniform_ratio, result_dir, result_prefix, parallel=False)
...
if __name__ == "__main__":
    ...
    for x,y in [(1,6),(2,2),(2,9),(3,1),(3,6),(3,7),(4,1),(4,7),(5,3)]:
        df = run_acas_experiment_parallel(x, y, spec_id, coef=0.05, uniform_ratio=0.0)
        all_results.append(df)
    summary_df = pd.concat(all_results, ignore_index=True)
    summary_df.to_csv("./results/acas_experiments/acas_parallel_all_summary.csv", index=False)
    print("所有实验已完成，汇总结果已保存。") 
    
    for x,y in [(1,6),(2,2),(2,9),(3,1),(3,6),(3,7),(4,1),(4,7),(5,3)]:
        df = run_acas_experiment_serial(x, y, spec_id, coef=0.05, uniform_ratio=0.0)
        all_results.append(df)
        
    summary_df = pd.concat(all_results, ignore_index=True)
    summary_df.to_csv("./results/acas_experiments/acas_serial_all_summary.csv", index=False)
    print("所有实验已完成，汇总结果已保存。") 